...
def get_technical_plot(df, index_name):
    ...
    axs[1].legend()
    plt.savefig(PATH + index_name + "_tecnical.jpg")
...
def get_strategy_plot(df, strategy_final, index_name):
    ...
    axs[1].plot(strategy_final['Date_Entry'], strategy_final['Cumulative Returns'], label='Cumulative Returns', c='grey')
...
def strategy(ticker, index_name):
    db = DBLoad()
    df = db.load(ticker)
    get_technical_plot(df, index_name)
    ...
    get_strategy_plot(df, strategy_final, index_name)
    return strategy_final, df


if __name__ == "__main__":
    strategy_final, df = strategy("HAL", "Hindustan Aeronautics Ltd")
    strategy_final
