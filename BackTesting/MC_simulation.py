...
def back_testing_mc(price_data, strategy):
    ...
    fitness = np.zeros((SIMULATIONS, 4))
    ...
    fitness[:, 3] = [result.get("Max draw-down") for result in results]
...
def plot_back_testing_results(fitness_ga, fitness_det):
    ...
    for i, title in enumerate(["Total Returns", "Performace Ratio", "Max Draw-down"]):
        ...
        axs[i].legend(["Deterministic strategy", "GA Optmized"])
...
if __name__ == "__main__":
    ...
    ga_strategy = dict(time_period={'ADXPeriod': 28, 'RSIPeriod': 7, 'EMAPeriod': 24},
